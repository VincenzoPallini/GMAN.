...
def load_data(args):
    ...
    # Tentativo di inferire la frequenza
    freq_str = pd.infer_freq(time)
    if freq_str is None:
        # Gestione di indici irregolari: calcolo la differenza media tra gli indici
        avg_diff = (time[1:] - time[:-1]).mean()
        freq_seconds = avg_diff.total_seconds()
    else:
        freq_seconds = pd.to_timedelta(freq_str).total_seconds()
